...
module Countdown = {
  ...
  let iso8601_of_ptime = (((y,m,d), ((hh, mm, ss), _))) => sprintf("%04d-%02d-%02dT%02d:%02d:%02dZ", y, m, d, hh, mm, ss);
  let date_or_error = (date) => ptime_of_iso8601(date) |> (fun
  | Some(d) => d
...
/* switch (ptime_of_iso8601(date)) {
  ...
  let t = {
    let encode = ({short, description, startDate, endDate}) =>
      [@implicit_arity] Ok(short, description, date_or_error(startDate), date_or_error(endDate));
    let decode = ((short, description, startDate, endDate)) => Ok({short, description, startDate: iso8601_of_ptime(Ptime.to_date_time(startDate)), endDate: iso8601_of_ptime(Ptime.to_date_time(endDate))});
    let rep = Caqti_type.(tup4(string, string, Caqti_type.ptime, Caqti_type.ptime));
...
let create_timer = (short, description, startDate, endDate, (module Db): (module Caqti_lwt.CONNECTION)) => Db.find(Q.create_timer, (short, description, startDate, endDate));
...
let countdown =
  Schema.(
    obj("countdown", ~fields=(db) =>
      [
        field("short", ~args=Arg.([]), ~typ=non_null(string), ~resolve=(db, p: Countdown.t) =>
          p.short
        ),
        field(
          "description",
        ...
        field(
          "startDate", ~args=Arg.([]), ~typ=non_null(string), ~resolve=(db, p: Countdown.t) =>
          p.startDate
        ),
        field(
          "endDate",
...
let testCountdown: Countdown.t =
  { short: "blah", description: "Diddy doo dah", startDate: "2018-03-24T12:00:00Z", endDate: "2018-03-30T12:00:00Z"};
    ...
    ~mutations=[
      io_field("createCountdown",
        ...
        ~resolve=(db, _, short, description, startDate, endDate) => {
          let resolvedShort = switch(short) {
          | Some(s) => s
          | None => RandomWord.get()
          };
          db >>= create_timer(resolvedShort, description, startDate, endDate) >|= (fun
        | Ok(timer) => timer
        | Error(err) => {
            Lwt_io.printf("Error: %s\n", Caqti_error.show(err));
            raise(Lookup_Error(err));
